...
def set_amountstock(new_value):
    global amountstock
    amountstock = round(amountstock, 2)

def update_cash_label():
    set_amountstock(amountstock)
...
#closes the welcome screen & opens up the main GUI
def mainWindow():
    welcome.destroy()
    orderingGUI()

# Adding a function to show the Range Trading Screen
...
#import a message box as apart of gui implementation on api screen
import tkinter as tk
from tkinter import messagebox
# function to handle submission
def submit():
    apiKey = api_key_entry.get()
    apiSecretKey = api_secretkey_entry.get()
    # Display the data
    messagebox.showinfo("User API Keys", f"API Key: {apiKey}\nAPI Secret Key: {apiSecretKey}") #"User API Keys", f"API Key: {api_key_entry}\nAPI Secret Key: {api_secretkey_entry}"

# Adding a function to show the api key updating Screen to connect to config.py
def api_update_screen():
    # Create a new window for the api key update
    api_window = tk.Toplevel(root)
    api_window.title('API Update')
    api_window.geometry('800x600')

    # Create a label for the screen title
    api_title_label = tk.Label(api_window, text="API Key Login", font=('TkDefaultFont', 18, 'bold'))
    api_title_label.grid(row=0, column=0, columnspan=4, padx=10, pady=10)

    #Create a description explaing that user will input their API key
    api_decsrp_label = tk.Label(api_window, text="Please insert your unique API key received from Alpaca", font=('TkDefaultFont', 12, 'bold'))
    api_decsrp_label.grid(row=0, column=0, columnspan=4, padx=10, pady=10)

    #---labels and entry fields---


    #api key entry label
    tk.Label(api_window, text="API KEY").grid(row=0, column=0)
    global api_key_entry #made vars global so they are accessible in submit()
    api_key_entry = tk.Entry(api_window)
    api_key_entry.grid(row=0, column=1)
    
    #api secret key entry label
    tk.Label(api_window, text="API SECRET KEY").grid(row=1, column=0)
    global api_secretkey_entry #made vars global so they are accessible in submit()
    api_secretkey_entry = tk.Entry(api_window)
    api_secretkey_entry.grid(row=1, column=1)

    # Submit button
    api_submit_button = tk.Button(api_window, text="Submit API Keys", command=submit)
    api_submit_button.grid(row=3, column=0, columnspan=2)

    # Call range trading and capture the results
    #results = range_trading()
    #print(results)
    #a func runs the results of range and then teh a func pritn sthe title and gives resulsts

# Open API update screen
api_update_button = tk.Button(root, text="Open API Update Screen", command=api_update_screen)
api_update_button.pack(pady=20)


#this function will accept user's specific api login
#def api_login():
    #print("hi")
